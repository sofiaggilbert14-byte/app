...
export function prepareNativeFullscreen(uri: string, headers: Record<string, string>, contentType?: string | null): void {
  native?.setFullscreenViewport();
  native?.prepareFullscreen(uri, headers, contentType ?? null);
}

export function prepareNativePreview(uri: string, headers: Record<string, string>, contentType?: string | null): void {
  native?.preparePreview(uri, headers, contentType ?? null);
}

export function setNativePreviewViewport(x: number, y: number, width: number, height: number): void {
  native?.setPreviewViewport(x, y, width, height);
}
...
export function selectNativeAudio(track?: NativePlaybackTrack | null, language?: string | null): void {
  if (track) native?.selectAudio(track.groupIndex, track.trackIndex);
  else native?.selectAudioLanguage(language ?? null);
}

export function selectNativeSubtitle(track?: NativePlaybackTrack | null, language?: string | null): void {
  if (track) native?.selectSubtitle(track.groupIndex, track.trackIndex);
  else if (language) native?.selectSubtitleLanguage(language);
  else native?.subtitlesOff();
}
...
export async function stopNativeFullscreen(releasePlayer = true): Promise<void> {
  await native?.stopFullscreen(releasePlayer);
}

export async function getNativePlaybackOwner(): Promise<NativePlaybackOwner> {
  return (await native?.getOwner()) ?? "none";
}

export function addNativePlaybackStateListener(
  listener: (event: { owner: NativePlaybackOwner; state: NativePlaybackState; reason?: string | null }) => void,
): () => void {
  const sub = emitter?.addListener("NativePlaybackState", listener);
  return () => sub?.remove();
}

export function addNativePlaybackTracksListener(
  listener: (event: { owner: NativePlaybackOwner; audio: NativePlaybackTrack[]; text: NativePlaybackTrack[] }) => void,
): () => void {
  const sub = emitter?.addListener("NativePlaybackTracks", listener);
  return () => sub?.remove();
}
