...
test("Media3 hot-applies decoder preferences while preserving bounded buffers", async () => {
  ...
  assert.match(source, /usePlayerCompatibilityPreferences\(\)/); assert.match(source, /usePlaybackBufferProfile\(\)/); assert.match(source, /const effectiveBufferProfile = bufferProfile \?\? savedBufferProfile/); assert.match(source, /compat\.media3AudioMode === "ffmpeg"/); assert.match(source, /compat\.media3Tunneling/); assert.match(source, /profile === "low_latency"/); assert.match(source, /profile === "stable"/); assert.match(source, /coordinatedCacheBudget/); assert.match(source, /Math\.min\(full\.maxBufferBytes, coordinatedCacheBudget\)/); assert.match(source, /mode === "preview"/); assert.match(source, /player\.audioMixingMode = mode === "preview" \? "mixWithOthers" : "doNotMix"/); assert.match(source, /player\.muted = muted/); assert.match(source, /if \(paused\) player\.pause\(\); else if \(playbackFocused\) player\.play\(\)/); assert.match(source, /publishTracks\(\)/); assert.match(source, /player\.subtitleTrack = selectedText/); assert.doesNotMatch(source, /vlcEngineKey|media3EngineKey|--stereo-mode=1|hardStop\(\)/);
});
...
test("real-device defaults keep automatic playback on Media3", async () => {
  ...
  assert.match(engine, /cachedPreference: PlayerEnginePreference = "media3"/);
  assert.match(engine, /PLAYER_ENGINE_KEY, "media3"/);
  assert.match(engine, /legacy `vlc` values are migrated to Media3/);
