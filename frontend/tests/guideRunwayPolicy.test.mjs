...
test("guide runway keeps seven warm pages on both sides of focus", () => {
...
test("guide runway clamps cleanly at both playlist boundaries", () => {
  ...
  assert.equal(top.at(-1), "channel-79");
  assert.equal(bottom[0], "channel-40");
  assert.equal(bottom.at(-1), "channel-119");
});

test("every power profile preserves the requested seven-page runway", () => {
  assert.deepEqual(guideRunwayPagesForProfile("weak"), { ahead: 7, behind: 7 });
  const weakDown = buildGuideRunwayIds(rows, 65, 10, 1, "weak");
  assert.equal(weakDown[0], "channel-0");
  assert.equal(weakDown.at(-1), "channel-119");
  assert.equal(weakDown.length, 120);
});
