...
// Serve pre-gzipped bytes from KV. Returns them compressed to clients that
// accept gzip (near-zero CPU); otherwise streams a decompressed copy.
async function serveGzip(env, key, acceptEncoding, maxAge) {
  ...
  // Rare fallback: client can't handle gzip — stream-decompress.
...
// current + next program for one channel, computed at REQUEST time (accurate).
...
export default {
  async fetch(request, env) {
    ...
    try {
      if (path === "/" || path === "/health") {
        return jsonResponse({ ok: true, service: "charm-iptv-api" }, { maxAge: 30 });
      }
